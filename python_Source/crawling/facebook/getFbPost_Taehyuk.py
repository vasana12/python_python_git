...
def getPostData(post, access_token, jsonResult):
    ...
    jsonResult.append({'post_id': post_id, 'message':post_message,
                       ...
                       'num_comments':post_num_comment, 'num_shared':post_num_shares,
...
def main():
    ...
    access_token = '378587619347850|h0aO-0xTQSBNz9E9H_cQC2GtONM'   #access_token 입력

    from_date = '2015-09-30'
    to_date = '2018-10-01'

    num_posts = 50
    # 반복 처리하는 부분, False일 경우, 반복 끝
    go_next = True
    jsonResult = []
    ...
    print("[%s] %s page_id is %s" % (datetime.datetime.now(), page_name, page_id))
